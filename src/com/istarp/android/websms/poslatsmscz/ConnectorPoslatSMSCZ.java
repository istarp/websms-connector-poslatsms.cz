...
public class ConnectorPoslatSMSCZ extends Connector {
	...
	private final static String API_ID = "023545id";
	...
	protected final void doSend(final Context context, final Intent intent) {
		...
		if (!isOnline()) 
			throw new WebSMSException(getStringResource(R.string.error_connection));
		else {
			if(user == null)
				throw new WebSMSException(getStringResource(R.string.error_user_not_find));
			else {				
	...
	private final ConnectorSpec createConnectorSpec() {
		...
		c.setAuthor(// .
				getStringResource(R.string.connector_poslatsmscz_author));				
		...
		c.addSubConnector("poslatsms.cz", name,				
		...
		if (p.getBoolean(Preferences.PREFS_ENABLED, false)) 
			if (getUserName().length() > 0 && getUserPassword().length() > 0) {
				...
				if(user !=null) 
					...
					connectorSpec.setReady(); //priprava pro kontrolu prihlaseni		
				}
			else
	...
	private final HttpResponse performHttpRequestForStatusLineUtils(final String url, final ArrayList<BasicNameValuePair> postData) throws IOException {
		return Utils.getHttpClient(url, null, postData, USER_AGENT, REFERER_URL, ENCODING);
	}
	...
	private final User loadUserInfo() {
		...
		nameValuePairs.add(new BasicNameValuePair("password", getHashPassword(getUserPassword(), HASH_TYPE)));
		nameValuePairs.add(new BasicNameValuePair("textsms", getHashPassword(getUserPassword(), HASH_TYPE)));	
				
		try {
			
			HttpResponse response = performHttpRequestForStatusLineForResponse(SMS_USER_INFO_URL, nameValuePairs);	
	...
	private final void sendSMS(String text, String phoneNumber) {
		...
		if (text.length() > 160)
			throw new WebSMSException(getStringResource(R.string.error_length,maxSMSLenght));
		...
		try {
			...
			HttpResponse response = performHttpRequestForStatusLineForResponse(SMS_PAGE_URL, nameValuePairs);	
			
			if (response.getStatusLine().getStatusCode() != HttpStatus.SC_OK) {
				throw new WebSMSException(getStringResource(R.string.error_sms_sending_result));
			}
			...
			//response state, everything except 3+ is correct
			if(userReponse.state == "3+") {
				throw new WebSMSException(getStringResource(R.string.error_service));
			}
